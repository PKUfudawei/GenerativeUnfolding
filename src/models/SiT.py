...
class ConditionEmbedder(nn.Module):
    ...
    def forward(self, condition):
        return self.condition_embedding(condition)
...
class SiTBlock(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, qk_norm=False, fused_attn=False):
        super().__init__()
...
class SiT(nn.Module):
    ...
    def __init__(
        ...
        **block_kwargs # fused_attn
        ...
        self.num_features = 6
        self.x_embedder = nn.Linear(1, hidden_size)  # 每个 feature 一个 token
        self.pos_embed = nn.Parameter(torch.zeros(1, self.num_features, hidden_size))
        ...
        self.cond_embedder = ConditionEmbedder(hidden_size) if use_conditions else None
    ...
    def initialize_weights(self):
        ...
        if self.cond_embedder is not None:
            nn.init.normal_(self.cond_embedder.condition_embedding[0].weight, std=0.02)
            nn.init.constant_(self.cond_embedder.condition_embedding[0].bias, 0)

            nn.init.normal_(self.cond_embedder.condition_embedding[2].weight, std=0.02)
            nn.init.constant_(self.cond_embedder.condition_embedding[2].bias, 0)
    ...
    def forward(self, x, t, r, cond=None, y=None):
        """
        ...
        x = self.x_embedder(x)           # (B, 6, D)
        x = x + self.pos_embed           # (B, 6, D)
        ...
        if self.num_classes is not None:
            ...
            c = c + y_embed
        if self.cond_embedder is not None:
            cond_embed = self.cond_embedder(cond)  # (N, D)
            c = c + cond_embed
